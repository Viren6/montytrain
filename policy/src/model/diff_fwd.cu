...
extern "C" __global__ void kernel(
    ...
    if (move.x != -1)
    {
        ...
        addf4(&val, reinterpret_cast<const float4*>(weights + hl_size * move.x)[loc_in_neurons]);

        if (move.y != -1)
        {
            addf4(&val, reinterpret_cast<const float4*>(weights + hl_size * move.y)[loc_in_neurons]);
        }
