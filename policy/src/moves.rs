...
fn see(pos: &Position, mov: &Move) -> i32 {
    let sq = usize::from(mov.to());
    assert!(sq < 64, "wha");

    // Piece that is *currently* on the from–square (after any promotion is applied)
    ...
    // Material swing after the first capture we are about to play
    let mut score = gain(pos, mov) - SEE_VALS[next];

    // If the immediate gain is already non‑negative we can return it – this
    // mirrors the old `true` short‑circuit while keeping identical semantics.
    if score >= 0 {
        return score;
    }

    // --- identical to the old code from here on ---------------------------------
    let mut occ = (pos.piece(Side::WHITE) | pos.piece(Side::BLACK))
        ^ (1 << mov.src())
        ^ (1 << sq);
    ...
    // `us` is the side that will reply after the first capture
    let mut us = pos.stm() ^ 1;

    let mut attackers =
        (Attacks::knight(sq) & pos.piece(Piece::KNIGHT)) |
    ...
    loop {
        // All of *our* pieces that can still capture the target square
        let our_attackers = attackers & pos.piece(us);
        if our_attackers == 0 {
            break; // no further captures possible
        }

        // Pick the least‑valuable attacker of the current side
        for pc in Piece::PAWN..=Piece::KING {
            let board = our_attackers & pos.piece(pc);
            if board > 0 {
                occ  ^= board & board.wrapping_neg(); // remove that piece
                next  = pc;
                break;
            }
        }

        // X‑ray attackers revealed after `next` vacated its square
        ...
        attackers &= occ; // only pieces still on the board may attack

        // “Recursive” SEE: opponent’s best response tries to minimise our gain
        score = -score - 1 - SEE_VALS[next];
        us    ^= 1; // switch side to move

        if score >= 0 {
            // Preserve original king‑recapture quirk
    ...
    // `score` now represents the final net material balance from the viewpoint
    // of the *original* side to move: positive = good, negative = bad.
    score
}
